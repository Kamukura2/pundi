...
The Android back fix was verified on the rebuilt APK: authenticated Accumulation dashboard -> Assets / Investment -> Android system Back returned to Accumulation without exiting to the launcher. In this remaining-QA run, modal Back closed the Data & Sync modal, nested page Back returned from Assets to the preceding Transactions/History view, and root Back returned the activity to the Android launcher without a crash or loop. Compact (`360x800`) and large (`430x932`) headless display checks rendered the authenticated shell and primary controls without obvious horizontal overflow or critical clipping. Transactions rendered successfully and Feedback opened with usable text entry, keyboard, and reachable submit control. The disposable Feedback submission was rejected by the runtime with `Authentication required.`; no feedback row was created. External-browser handoff was not reached. A valid synthetic backup passed client validation and reached the replacement confirmation, but confirmation failed with `permission denied for table app_settings`; the invalid-fixture selection did not produce a definitive rejection capture. Synthetic fixture files were removed from the emulator Downloads directory. Android `versionCode 80701`; web/runtime and Android `versionName` remain `8.7.0`. Android Alpha therefore remains pending the failed/unverified runtime gates rather than COMPLETE.
