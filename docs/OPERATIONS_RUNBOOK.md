...
- Product version is `8.3.4`, sourced from `package.json`, lockfile, README heading, document title, and the service-worker shell cache key.
- Build identity is generated by Vite from the current Git short SHA and rendered as `Pundi v8.3.4 · build <sha>`; `PUNDI_BUILD_ID` may override it only in an explicitly controlled build.
...
- The service worker uses `pundi-shell-v8.3.4`, excludes `/api/*`, synchronously clones responses before asynchronous caching, and deletes old shell caches during activation.
