...
const supabase = await readFile("src/lib/supabase.js", "utf8");
const syncManager = await readFile("src/sync/sync-manager.js", "utf8");
...
assert.match(syncManager, /getAuthenticatedSession\(\)/, "SyncManager must refresh the persisted session before native cloud writes.");
