...
const origin = process.env.PUNDI_PRODUCTION_URL || "https://pundi-silk.vercel.app";
const expectedBuild = process.env.PUNDI_EXPECTED_BUILD || execFileSync("git", ["rev-parse", "--short=7", "HEAD"], { encoding:"utf8" }).trim();
const fetchText = async path => { const response=await fetch(origin+path,{cache:"no-store"}); return {response,text:await response.text()}; };
...
for (const route of routes) {
  ...
  if(route==="/") assert.match(text,/Pundi v8\.3\.4/);
  ...
  if(route==="/sw.js") assert.match(text,/pundi-shell-v8\.3\.4/);
}
...
assert.match(bundle,new RegExp(`Pundi v8\\.3\\.3 · build ${expectedBuild}`));
assert.doesNotMatch(bundle,/SUPABASE_SERVICE_ROLE_KEY|service_role|smoke password|cvfinance\.supabase\.co/i);
