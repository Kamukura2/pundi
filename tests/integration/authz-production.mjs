...
const cfg=await (await fetchWithTimeout(origin+"/api/config",{cache:"no-store"})).json();assert.match(cfg.supabaseUrl,/ndeycwoyjwyntjkgbzlz/);
const unauth=await fetchWithTimeout(origin+"/api/admin",{cache:"no-store"});assert.equal(unauth.status,401);
async function role(e,p,expected){const client=createClient(cfg.supabaseUrl,cfg.supabaseAnonKey,{global:{fetch:fetchWithTimeout}});const {data,error}=await client.auth.signInWithPassword({email:e,password:p});assert.ifError(error);const r=await fetchWithTimeout(origin+"/api/admin",{headers:{Authorization:`Bearer ${data.session.access_token}`},cache:"no-store"});assert.equal(r.status,expected);await client.auth.signOut();}
