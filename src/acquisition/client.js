...
import { apiUrl, isNativeRuntime } from "../lib/runtime.js";
...
export async function persistSignupAttribution(user) {
  if (!user?.id || isNativeRuntime()) return;
  ...
  await fetch(apiUrl("/api/acquisition"),{method:"POST",cache:"no-store",headers:{Authorization:`Bearer ${session.access_token}`,"Content-Type":"application/json"},body:JSON.stringify({action:"signup_attribution",source:acquisitionSource(),landing_path:landingPath})});
}
