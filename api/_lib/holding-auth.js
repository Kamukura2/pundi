...
export async function resolveAuthorizedUser(request) {
  ...
  if (error || !user) throw Object.assign(new Error("Authentication required."), { code:"unauthorized", status:401 });
...
export async function resolveAuthorizedHolding(request, holdingId) {
  ...
  const { supabase } = await resolveAuthorizedUser(request);
