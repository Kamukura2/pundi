...
try {
  ...
  if ([string]::IsNullOrWhiteSpace($env:PUNDI_ADMIN_SMOKE_EMAIL) -or [string]::IsNullOrWhiteSpace($env:PUNDI_ADMIN_SMOKE_PASSWORD)) { throw 'Smoke credential variables are missing.' }

  $stdoutFile = Join-Path $HealthDir (".stdout-{0}.tmp" -f [guid]::NewGuid())
  $stderrFile = Join-Path $HealthDir (".stderr-{0}.tmp" -f [guid]::NewGuid())
  try {
    $process = Start-Process -FilePath 'npm.cmd' -ArgumentList @('run','test:admin-smoke') -WorkingDirectory $Repo -WindowStyle Hidden -Wait -PassThru -RedirectStandardOutput $stdoutFile -RedirectStandardError $stderrFile
